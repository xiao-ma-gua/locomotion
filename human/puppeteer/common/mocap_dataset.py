...
import bisect
from typing import Dict, Sequence, Text, Tuple, Optional, Union
import h5py
import itertools
import collections
...
from envs.tracking import select_clips, TRACKING_OBSERVABLES
...
class ExpertDataset(Dataset):
	def __init__(
		...
		ref_steps: The reference steps to use for tracking.
		"""
		...
		for fname in self._hdf5_fnames:
			with h5py.File(fname, 'r') as dset:
				if clip_ids is None: # get every clip ID in the dataset
					...
					key = [k for k in dset.keys() if k.startswith('CMU')][0]
					self._clip_snippets.append(tuple([key for k in clip_ids if k in key]))
		...
		with h5py.File(self._hdf5_fnames[0], 'r') as dset:
			if ref_steps is None:
				...
				self._ref_steps = np.array(ref_steps)
			obs_ind_dset = dset['observable_indices/walker']
			self._all_observable_indices = {
				f"walker/{k}" : obs_ind_dset[k][...] for k in obs_ind_dset
			}
			self._observable_indices = dict()
			for k, indices in self._all_observable_indices.items():
				if 'reference' in k: # only extract desired number of reference steps
					indices_per_step = len(indices) // 5
					self._observable_indices[k] = indices[:indices_per_step * len(self.ref_steps)]
				else:
					self._observable_indices[k] = indices
	...
	def _set_stats(self):
		...
		self._proprio_mean = metrics['proprio_mean'].astype(np.float32)
		self._proprio_var = metrics['proprio_var'].astype(np.float32)
		if self._return_mean_act:
			self._act_mean = metrics['mean_act_mean'].astype(np.float32)
			self._act_var = metrics['mean_act_var'].astype(np.float32)
		else:
			self._act_mean = metrics['act_mean'].astype(np.float32)
			self._act_var = metrics['act_var'].astype(np.float32)
	...
	def _extract_observations(self, all_obs: np.ndarray, observable_keys: Sequence[Text]):
		return {k: all_obs[..., self.observable_indices[k]] for k in sorted(observable_keys)}
...
class MocapDataset(ExpertDataset):
	...
	def __init__(
		...
		print(f'Loading MoCap dataset with {len(clip_ids)} clips')
		super().__init__(
			hdf5_fnames,
			observables,
			metrics_path,
	...
	def _getitem(self, dset, idx):
		...
		if end_idx == act_dset.shape[0]-1: # end of episode
			terminated = np.array(float(self._early_terminations[dset_idx][clip_idx]), dtype=np.float32)
		else:
			terminated = np.array(0., dtype=np.float32)

		return obs, act, rew, terminated
...
class MocapBuffer():
	...
	def __init__(self, cfg):
		...
		self._dataset = MocapDataset(
			hdf5_fnames=glob(os.path.join(cfg.data_dir, '*.hdf5')),
			observables=TRACKING_OBSERVABLES,
			metrics_path=os.path.join(self.cfg.data_dir, 'dataset_metrics.npz'),
			clip_ids=select_clips(cfg),
			min_seq_steps=cfg.horizon,
			max_seq_steps=cfg.horizon,
			ref_steps=[1],
		)
	...
	def _permute(self, *args, dims=(1,0,2)):
	...
	def sample(self):
		...
		try:
			...
			self._iter = iter(self._dataloader)
			obs, action, reward, terminated = next(self._iter)
		obs, action, reward = self._permute(obs, action, reward.unsqueeze(-1))
		return self._to_device(obs, action, reward, terminated.unsqueeze(-1))
